...
class eventMaker: 
	...
	def generalize_noun(self, word, tokens, sentence_NE_list):
		...
		for sent_NE in curr_sent_NEs:
			if word.lower() == sent_NE or word.lower() in sent_NE.split(" "): #this noun is a subset of one of the pre-tagged NEs
	...
	# given a parse of a sentence, extract the event(s)
		...
		for sent_num, sentence in enumerate(all_json):
			...
			for (a,b,c,d) in zip(subjects, verbs, objects, modifiers):
				...
				if returningEvent: return returningEvent_orig, returningEvent,event_nouns
				else: return [],[], None
