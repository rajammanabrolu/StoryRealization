...
class VAERetriever(Module):
    ...
    def ret_lsh(self, veclist, lsh, topk=100, startat=0):
        ex = [lsh.get_nns_by_vector(vec, topk+startat)[startat:] for vec in veclist]
        
        dist = [lsh.get_nns_by_vector(vec, topk+startat, include_distances=True)[1][startat:] for vec in veclist]
    ...
    def ret_idx(self, in_data, lsh, train_mode=True):
        ...
        topk_ret, dist = self.ret_lsh(encoded_np, lsh)
        return topk_ret, dist
