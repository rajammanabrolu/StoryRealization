...
parser.add_argument('--data', type=str, default='./data/new_abstracted_scifi',
                    help='location of the data corpus')
parser.add_argument('--model', type=str, default='LSTM',
                    help='type of recurrent net (LSTM, QRNN)')
parser.add_argument('--checkpoint', type=str, default='./abs_gen4.pt',
...
parser.add_argument('--input_event_file', type=str, default='event_data/corrected-5tuple-full-genEvent-genSent-Test_input.txt',
                    help='input events')
# parser.add_argument('--input_event_file', type=str, default='event_data/fullGenE2S-singleEvent-NewVerb-Test_input.txt',
#                     help='input events')
parser.add_argument('--num_args', type=int, default=5, help='5arg events or 4arg')
...
def predict_beam(next_word, model, forwards=True, maxlen=20, noun=True, ending=False, sample=True):
    ...
    for i in range(maxlen):
        ...
        if next_word == '<eos>' or (doc[0].pos_ == "CCONJ" and got_conj) or doc[0].pos_ == "VERB" or '-' in next_word or (forwards == False and (last_pos == "DET" or last_pos == "ADP") and isNoun(next_word)): #check if verb
            keep_going = False
            continue
        if doc[0].pos_ == "CCONJ":
            got_conj = True
...
def main(args):
    ...
    pure_out = open('test_pure_output.txt', 'w')
    loss_file = open('test_loss_output.txt', 'w')
    ...
    with open(args.outf, 'w') as outf:
            ...
            for event in generalized_events:
                ...
                if FRAME == None or FRAME == [] or args.sample == False:
                    ...
                    if (size == 0):
                        ...
                        pure_out.write(' '.join(sent) + '\n')
                        loss_file.write(str(1) + '\n')
                        sent_lengths.append(len(sent))
                        all_losses.append(1)
                    else:
                        outf.write(' '.join(sent) + ';' + str(' '.join(event)) + ';loss:' + str((total_loss/size)/max_loss)+ '\n')
                        pure_out.write(' '.join(sent) + '\n')
                        loss_file.write(str((total_loss/size)/max_loss) + '\n')
                        sent_lengths.append(len(sent))
                        all_losses.append(total_loss/size)
                        real_losses.append(total_loss/size)
                    continue
                #print (FRAME)
                num_nps = 0
                for pos in FRAME:
        ...
        print ("MEAN SENT LENGTH: ")
        print (mean(sent_lengths))
