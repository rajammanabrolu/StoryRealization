...
def predict_beam(next_word, model, forwards=True, maxlen=20, noun=True, ending=False, sample=True):
    ...
    for i in range(maxlen):
        ...
        if forwards:
            model_output, _, hidden = model(word_input, hidden, return_h=False)
        else:
            _, model_output, hidden = model(word_input, hidden, return_h=False)
    ...
    with open(args.outf, 'w') as outf:
        ...
        for line in tqdm(lines):
            ...
            for event in generalized_events:
                ...
                if FRAME == None or FRAME == [] or args.sample == False:
                    ...
                    if event[0] != "EmptyParameter":
                        if re.match('<.*?>.*', event[0]):
                            ...
                            sent.append(predict_beam(event[0], model, forwards=False, maxlen=1, ending=False, sample=False)[0])
                            sent.append(event[0])
                    ...
                    if event[2] != "EmptyParameter":
                        if re.match('<.*?>.*', event[2]):
                            ...
                            sent.append(predict_beam(event[2], model, forwards=False, maxlen=1, ending=False, sample=False)[0])
                            sent.append(event[2])
                    ...
                    if event[3] != "EmptyParameter":
                        if re.match('<.*?>.*', event[3]):
                            ...
                            pre = predict_beam(event[3], model, forwards=False, maxlen=1, ending=False, sample=False)[0]
                            if pre != event[4]:
                                sent.append(pre)
                            sent.append(event[3])

                    outf.write(' '.join(sent) + ';' + str(event) + '\n')
                    continue
                #print (FRAME)
                num_nps = 0
                for pos in FRAME:
                    ...
                    if pos == 'NP' or pos == 'S':
                        ...
                        num_nps += 1
                        beam.append(word)
                        beam = predict_beam(word, model, forwards=False, maxlen=3) + beam
                        if num_nps > 0:
                            beam = beam + predict_beam(word, model, forwards=True, maxlen=3, ending=True)
                        else:
                            beam = beam + predict_beam(word, model, forwards=True, maxlen=3)
                ...
                #rint (' '.join(sent))
                #print ('*******************************************')
                outf.write(' '.join(sent) + ';' + str(event) + ';' + str(FRAME) + '\n')
