...
class MCTSBeamSearchDecoder(object):
    """Beam Search decoder."""
    ...
    def decode_batch(self, idx):
        ...
        beam = [
            MonteCarlo(beam_size, self.tgt_dict, self.id2word_src, self.trg['id2word'],self.config['data']['max_trg_length'], self.model, cuda=True)
    ...
    def translate(self):
        ...
        #with open("e2s-monte-carlo-output.txt", "a") as f:
            for p, score in zip(all_preds, scores):
                ...
                self.allsents.append(p.replace('<s>', '').replace('</s>', '.').strip())
...
#******************************************
class FSMBeamSearchDecoder(object):
    """Beam Search decoder."""
    ...
    def decode_batch(self, idx):
        ...
        beam = [
            Beam(beam_size, self.tgt_dict, self.id2word_src, self.trg['id2word'], cuda=True)
            for k in range(batch_size)
        ]
...
class BeamSearchDecoder(object):
    ...
    def translate(self):
        ...
        for j in itr:
            ...
            all_hyp_inds = [[x[0] for x in hyp] for hyp in hypotheses]
            all_preds = [
                ' '.join([self.trg['id2word'][x.item()] for x in hyp])
                for hyp in all_hyp_inds
            ]
    ...
    def translate(self):
        ...
        for j in xrange(
            ...
            for sentence_pred, sentence_real, sentence_real_src in zip(
                ...
                #trg_gold += all_gold
                for p in preds:
                    self.allsents.append(" ".join(p).replace('</s>', '').strip())
